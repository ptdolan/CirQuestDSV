...
if(length(fileList>0)){
  ...
  dir.create(paste(rootDir,figureDir,sep=""),showWarnings = F)
  dir.create(paste(rootDir,outputDir,sep=""),showWarnings = F)
  dir.create(paste(rootDir,entropyDir,sep=""),showWarnings = F)
  ...
  for (q20file in fileList){ #Read annotated CirSeq output file
    ...
    q20$genotype<-as.factor(strsplit2(name[10],"_")[1])
    q20$rep<-as.factor(strsplit2(name[10],"_")[3])
    passage<-as.numeric(strsplit2(name[11],"p")[2])
    q20$passage<-passage
    #Name Interpretation
    ...
    ### Entropy
    Qh<-Q+geom_point(aes(ntpos,HsN),stat="identity")+
      ylim(0,.3)+
...
#   
  ...
  distDF<-data.frame(distDF,genotype=strsplit2(rownames(distDF),split = "Rep")[,1])
