...
library(ggplot2)
library(reshape2)
rootDir<-"~/Research/CirSeq/Hong/Download032716/NewDownload/Renamed/" #put input directory here.
########## plot frequencies ##########
...
if(length(fileList>0)){
  ...
  for (q20file in fileList){ #Read annotated CirSeq output file
    ...
    name<-strsplit2(name[length(name)],split = "Q20threshold_")[2]
    #q20$label<-as.factor(name)
    q20$genotype<-as.factor(strsplit2(name,"_P")[1])
    q20$rep<-NULL
    q20$passage<-passage<-as.numeric(strsplit2(name,"_P")[2])
    #Name Interpretation
    headerInfo<-name
    headerVec<-strsplit2(headerInfo,"_")
    ...
    PU<-Q+geom_hline(aes(yintercept = FILTER),lty=3,color="grey")+
      ...
      scale_color_brewer(palette = "Set1")+ggtitle(headerInfo)
...
#   distDF<-data.frame(distDF,genotype=rownames(distDF)
...
#   ggplot(PCDF)+geom_point(aes(PC1,PC2,color=genotype))+scale_color_brewer(palette="Paired")#+geom_text(aes(PC1,PC2,label=passage),nudge_x = -0.01)
#   ggplot(PCDF)+geom_point(aes(PC5,PC6,color=genotype))+scale_color_brewer(palette="Paired")#+geom_text(aes(PC5,PC6,label=passage))
...
#   
#   heatmap(as.matrix(Dist),scale='none')
#   
#   
...
#   
  ...
  ggsave(filename = paste(rootDir,figureDir,"dir_sqrt_TrajPlot.pdf",sep=""),
         device = 'pdf',TT+scale_y_sqrt(breaks=c(0.00001,0.0001,.001,.01,.05,.1,.2,.4,.6,.8,1))+facet_wrap(~header))
  
  ggsave(filename = paste(rootDir,figureDir,"dir_log_TrajPlot.pdf",sep=""),
         device = 'pdf',TT+scale_y_log10(breaks=c(10**c(-6:0)))+facet_wrap(~header))
